...
class Consultup_Customizer_Notify {
	...
	public function consultup_customizer_notify_dismiss_recommended_action_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			if ( get_theme_mod( 'consultup_customizer_notify_show' ) ) {

				$consultup_customizer_notify_show_recommended_actions = get_theme_mod( 'consultup_customizer_notify_show' );
				switch ( $_GET['todo'] ) {
					case 'add':
						$consultup_customizer_notify_show_recommended_actions[ $action_id ] = true;
						break;
					case 'dismiss':
						$consultup_customizer_notify_show_recommended_actions[ $action_id ] = false;
						break;
				}
				echo esc_html($consultup_customizer_notify_show_recommended_actions);
				
			} else {
				$consultup_customizer_notify_show_recommended_actions = array();
				if ( ! empty( $consultup_customizer_notify_recommended_actions ) ) {
					foreach ( $consultup_customizer_notify_recommended_actions as $consultup_lite_customizer_notify_recommended_action ) {
						if ( $consultup_lite_customizer_notify_recommended_action['id'] == $action_id ) {
							$consultup_customizer_notify_show_recommended_actions[ $consultup_lite_customizer_notify_recommended_action['id'] ] = false;
						} else {
							$consultup_customizer_notify_show_recommended_actions[ $consultup_lite_customizer_notify_recommended_action['id'] ] = true;
						}
					}
					echo esc_html($consultup_customizer_notify_show_recommended_actions);
				}
	...
	public function consultup_customizer_notify_dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			$consultup_lite_customizer_notify_show_recommended_plugins = get_theme_mod( 'consultup_customizer_notify_show_recommended_plugins' );

			switch ( $_GET['todo'] ) {
				case 'add':
					$consultup_lite_customizer_notify_show_recommended_plugins[ $action_id ] = false;
					break;
				case 'dismiss':
					$consultup_lite_customizer_notify_show_recommended_plugins[ $action_id ] = true;
					break;
			}
			echo esc_html($consultup_customizer_notify_show_recommended_actions);
		}
