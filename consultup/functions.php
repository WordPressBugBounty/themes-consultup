<?php define( 'CONSULTUP_THEME_DIR', get_template_directory() . '/' );
	...
	require( $consultup_theme_path . '/customize/consultup_customize_archive.php');
...
function consultup_widgets_init() {
	...
	register_sidebar( array(
		...
		'description'   => '',
		'before_widget' => '<div id="%1$s" class="col-md-'.$consultup_footer_column_layout.' rotateInDownLeft animated consultup-widget %2$s">',
		'after_widget'  => '</div>',
		'before_title'  => '<h6>',
		'after_title'   => '</h6>',
	) );
...
if (!function_exists('consultup_edit_link')) :

    function consultup_edit_link($view = 'default')
    {
        global $post;
		edit_post_link(
			sprintf(
				...
				/* translators: %s: Name of current post. Only visible to screen readers */
					__('Edit <span class="screen-reader-text">%s</span>', 'consultup'),
					array(
						'span' => array(
							'class' => array(),
						),
